...
export function UserSwitcher({ current, users }: { current: User; users: User[] }) {
  ...
  return (
    <>
      ...
      <Ariakit.Popover
        className="border-divider bg-surface dark:border-divider-dark dark:bg-black z-50 w-60 overflow-hidden rounded-xl border shadow-xl outline-none"
        gutter={8}
        portal
        store={popover}
      >
        ...
        <form action={signOut} className="border-divider dark:border-divider-dark border-t p-1">
          <button
            className="text-muted hover:bg-card hover:text-black dark:hover:bg-card-dark flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition-colors dark:hover:text-white"
            type="submit"
          >
