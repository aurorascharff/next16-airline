...
export async function DestinationDetails({ slug }: { slug: string }) {
  ...
  return (
    <article>
      <Link className="text-muted hover:text-accent inline-flex items-center gap-2 text-sm font-medium" href="/">
        <ArrowLeft className="size-4" /> Overview
      </Link>
      <div className="mt-5 grid overflow-hidden rounded-2xl lg:grid-cols-[1.05fr_0.95fr]">
        <div className="bg-accent p-7 text-white sm:p-10">
          ...
          <Link
            className="mt-10 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-black"
            href={`/search?to=${destination.airport}`}
          >
            Search flights <ArrowRight className="size-4" />
          </Link>
        </div>
        <div className="border-divider bg-surface dark:border-divider-dark dark:bg-black flex min-h-64 flex-col justify-between border p-7 sm:p-10">
...
export function DestinationDetailsSkeleton() {
  return <Skeleton className="mt-10 h-[28rem] rounded-2xl" />;
}
